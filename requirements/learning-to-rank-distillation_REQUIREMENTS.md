...
The single biggest change from the original spec: **no component should import Expedia-specific column names.** Every dataset — RecTour, a future second adapter, a synthetic generator — implements the same interface and everything downstream (teacher, student, distillation, reranking, governance) is written against that interface only.
...
Everything in Sections 3–6 is written against `RankingExample`, never against raw Expedia fields directly. Raw fields are mapped to this schema inside `adapters/rectour.py` only.

---

## 2. Dataset — Expedia Group RecTour Research Dataset (2021)
...
### 2.1 Known schema fields to map into `RankingExample`
...
- [ ] `RankingExample` schema defined; zero dataset-specific logic outside `adapters/`
- [ ] RecTour 2021 adapter implemented, field mapping confirmed against current documentation (not the 2021 slide deck alone)
- [ ] Teacher, student (KD), and student (no-KD) all trained and reproducible from a clean clone
- [ ] Benchmark table + quality-vs-latency Pareto plot
- [ ] Marketplace exposure-fairness metric implemented and plotted as a second Pareto frontier
- [ ] Promotion gate implemented as code, with a logged decision
- [ ] `README.md` complete (motivation, architecture, reproduction, Design Decisions, What This Doesn't Cover, link to ROADMAP)
- [ ] `ROADMAP.md` published (see companion document)
